...
1. The server has nothing but a nginx proxy and docker-compose (plus a thin "deployer" service used in point 5 below)
...
2. If you do not have non-root SSH access, you can add a sudo user with these (sample) simple commands -
    - __adduser rockyj__
...
1. Run tests & build your project
...
Once we know this we can update our docker-compose.yml file with the image names and start the docker-compose process with -
